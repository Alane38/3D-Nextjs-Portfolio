...
export const RacingVehicle = forwardRef<RapierRigidBody | null, VehicleProps>(
  ({ position, rotation }, ref) => {
    ...
    wheelsRef.current = wheels.map(
      (_, i) => wheelsRef.current[i] ?? new THREE.Object3D(),
    );
        ...
        >
          ...
          {wheels.map((wheel, index) => (
            <group
              key={index}
              ref={(ref) => {
                if (ref) wheelsRef.current[index] = ref;
              }}
