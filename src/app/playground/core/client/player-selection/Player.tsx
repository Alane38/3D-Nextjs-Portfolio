...
export const Player = () => {
  ...
  useEffect(() => {
    ...
    Object.keys(flattenedCharactersData).forEach((id) => {
      characterRefs.current[id] =
        characterRefs.current[id] || createRef<CharacterRef>();
    });

    // Add vehicle ref
    characterRefs.current["vehicle"] =
      characterRefs.current["vehicle"] || createRef<CharacterRef>();
  }, []);
    ...
    const checkAndSetCamera = () => {
      ...
      if (!currentRef?.current) {
        if (attempts >= MAX_ATTEMPTS) {
          console.warn(
            "Failed to find character reference after multiple attempts",
          );
  ...
  return (
    <>
      ...
      {/*  Render fisrt-person characters  */}
